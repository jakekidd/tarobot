...
Design doc for the antechamber AI pipeline. The user asked a senior-eng-tier
question: how should the antechamber's AI labor be broken up so latency is
...
Once the dowser owns hypothesis management, the investigator's job
...
- Should land first; feeds dowser + investigator.
...
type InvestigatorInput = {
  profile: AntechamberProfile;
  hypotheses: Hypothesis[];        // live, post-dowser
  suspicions: string[];            // dowser's leads
...
async function onAnswer(pick) {
  ...
  const dowserP  = earP.then(ear => fireDowser(state, ear).then(applyDowserToState));
  const investigatorP = earP.then(ear => fireInvestigator(state, ear).then(appendToQueue));
  // Dowser + investigator both wait on ear (small + fast), then run in
  // parallel. ear lands in ~400ms; dowser + investigator land in ~2s.
  // total wall-clock: ~2s, hidden behind queue lookahead.
}
```

Critical: **investigator doesn't wait for dowser**. Dowser writes
...
- Ear fails → dowser + investigator still fire with no observation.
  Slight prompt degradation, no engine break.
- Dowser fails → state isn't updated, investigator still appends.
  User sees no change.
...
```
src/pipeline/antechamber/agents/
  ear.ts                   ← new
  dowser.ts             ← new (was: split from observer.ts)
  investigator.ts          ← trim; remove option-modify reasoning
  compiler.ts              ← unchanged

src/pipeline/antechamber/prompts/
  ear.ts                   ← new (~400 words system prompt)
  dowser.ts             ← new (~800 words; this is where Clue lives)
  investigator.ts          ← trim to ~400 words

src/pipeline/antechamber/schemas.ts
  EarOutputSchema          ← new
  DowserOutputSchema    ← rename ObserverOutputSchema
  InvestigatorOutputSchema ← trim

src/pipeline/antechamber/engine.ts
  spawnAgentsForPick()     ← replaces fireObserver + spawnInvestigator
                             coordinates ear → dowser + investigator
```
...
- `Observer` → `Dowser` is a near-pure rename + scope trim.
...
2. **Dowser as a first-class concept.** "Play dowser" is now a
...
4. **Visible decomposition.** Debug overlay can show "ear: done /
   dowser: in flight / investigator: appended". The user can see
