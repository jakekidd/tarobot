...
/** Rejection-without-correction streak — last K consecutive assertion
...
export function isRejectionWithoutCorrectionStreak(picks: readonly PickEvent[]): boolean {
  const tail = recentAssertionPicks(picks, REJECTION_WITHOUT_CORRECTION_STREAK_THRESHOLD);
...
function recentAssertionPicks(
  _picks: readonly PickEvent[],
  _limit: number,
): readonly PickEvent[] {
  // Phase 2: no assertion instrument exists yet. Same shape as above.
  return [];
}
