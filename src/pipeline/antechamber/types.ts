...
//   which the observer single-writes and the diviner + coverage
//   read.
...
// - EngineState.diviner_log dropped (scratchpad lives per-call now).
...
export type AnswerFormat =
  ...
  | 'relationship_status'
  | 'fork'
  /** v3 diviner-emitted instrument formats. carry an `instrument`
...
export type CastMember = {
   ...
   *  relationship_pick "who specifically?" follow-up. The diviner is
 ...
 *  Built incrementally by the diviner across the antechamber. Its slots
 ...
 *
 *  When no clear fork emerges from the antechamber, the diviner falls
...
/** Empty StoryObject — initial value before the diviner has built anything. */
...
// (ObserverOutput, DivinerOutput, LadderRung deleted in Phase 3 —
// the v2 schemas live in agents/observer/schema.ts and
// agents/diviner/schema.ts; types inferred via z.infer.)
...
export type QueueItem = {
   ...
   *  Phase 4 diviner/crowd/interrogator pipeline rather than authored
  ...
  /** v3 diviner-emitted instrument payload. When set, the queue
...
export type EngineState = {
   ...
   *  is the sole writer; diviner + coverage read. Story + held
  ...
  // diviner_log REMOVED in v2. The diviner's scratchpad lives
  ...
  /** Interrogation pivot: the unified narrative the diviner reads.
  ...
  /** The diviner's accumulated thinking transcript across calls.
   *  Appended every diviner output (the free-form section before
   *  ===HYPOTHESIS===). Surfaces back into the next diviner payload
   *  so reasoning stays continuous. */
  diviner_thinking: string;
  /** Legacy trajectory log of diviner hypotheses — populated for
  ...
  /** Queued guesses the diviner has emitted but the user hasn't
...
/** A diviner-emitted guess queued for the user. The diviner
...
export type QueuedGuess = {
  ...
  idx: number;
  statement: string;
  /** The diviner's current hypothesis when this guess was emitted —
  ...
  /** When the diviner emitted this guess (post-opener turn count). */
...
export type PipelineContext = {
  ...
  question: string;
  /** Options actually shown to the user (post-diviner-edit). */
