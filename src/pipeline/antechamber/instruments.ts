...
/** The subject's response to an guess. */
export type GuessResult = {
  direction: 'warm' | 'cold' | 'hot';
  correction?: string;
};

export type Instrument = GuessInstrument;

const PREFIXES = ['cold', 'warm', 'hot'] as const;
type Prefix = (typeof PREFIXES)[number];

export function parseGuessAnswer(answer: string | string[]): GuessResult | null {
...
export function encodeGuessAnswer(result: GuessResult): string {
