...
// drops the user straight into the interrogation phase: assertion →
// COLD/WARM/HOT → next assertion. Strict serialization (lookaheadCap=1)
// — no speculative ahead-of-time generation. Soft ceiling raised so
// the detective can cook unbounded.
//
// The view is single-column, dense, lab-equipment-flat. Top: session
// status + new-session button. Below: current assertion + three
// buttons + correction follow-up. Below that: thinking trace,
// hypotheses, transcript. The point of this view is fast iteration
// on the detective prompt — refine, restart, refine.
...
export function Detective({ apiKey }: Props) {
  ...
  const inInterrogation = state.stage === 'questions'
    ...
    && (currentQ?.format === 'assertion' || state.thinking);
  ...
  return (
    <Stack gap={5}>
      ...
      <Panel
        title="thinking"
        meta={<span className="bench__text-mono bench__text-sm">{state.detective_thinking ? `${state.detective_thinking.length} chars` : '0 chars'}</span>}
      >
        <Stream
          text={state.detective_thinking}
          emptyHint={inInterrogation ? '(detective is forming first thoughts...)' : '(bootstrapping persona...)'}
          maxHeight={420}
        />
      </Panel>
      <Panel
...
function SessionBar({
  ...
  const assertionsAsked = state.transcript.filter((e) => e.kind === 'assertion').length;
  return (
    <div className="bench__run-controls">
      <Row gap={3}>
        ...
        <span className="bench__text-mono bench__text-sm bench__text-muted">
          {assertionsAsked} assertion{assertionsAsked === 1 ? '' : 's'} asked
        </span>
...
function InteractionArea({
  ...
  // here. Once we land in 'assertion' format, show the COLD/WARM/HOT
  // interface.
      ...
      <div className="bench__qcard">
        ...
        <div className="bench__qcard-q bench__text-muted">
          {state.stage === 'awaiting_intention'
            ? 'detective handed off — interrogation complete'
  ...
  if (currentQ && currentQ.format === 'assertion' && currentQ.instrument?.kind === 'assertion') {
    return (
      <div>
        <div className="bench__qcard-eyebrow">assertion · A{state.assertion_queue[0]?.idx ?? '?'}</div>
        <div className="bench__assertion-text">{currentQ.instrument.statement}</div>
...
function TranscriptLog({ state }: { state: EngineState }) {
  // Only show assertions + responses in this view — the openers and
  ...
  const entries = state.transcript.filter(
    (e) => e.kind === 'assertion' || e.kind === 'response'
  );
  if (entries.length === 0) {
    return <Empty>no assertions yet</Empty>;
  }
  return (
    <div className="bench__transcript">
      {entries.map((e, i) => {
        if (e.kind === 'assertion') {
          return (
            <div key={i} className="bench__transcript-entry bench__transcript-entry--assertion">
              <div className="bench__transcript-q">A{e.assertion_idx}. {e.statement}</div>
            </div>
          );
        }
