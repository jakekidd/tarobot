// Antechamber UI — thin shell over the AntechamberEngine.
//
...
//   - handoff: on antechamber close, give the ready Seer to App
//
...
import { WarmColdChoice } from './choices/WarmColdChoice';
...
} from './antechamber/centenarian';
import { parseBirthDate } from '../pipeline/astrology';
import { NameForm } from './antechamber/NameForm';
import { IntentForm } from './antechamber/IntentForm';
import { RelationshipStatusForm } from './antechamber/RelationshipStatusForm';
...
import { findPeopleMatchingName, type ReturningMatch } from '../pipeline/antechamber';
...
import { publishAntechamberState } from '../debug/antechamberStateBus';
...
type Props = {
  ...
  /** Antechamber hands off a ready Seer (intro pre-built) instead of a brief. */
  onComplete: (seer: Seer) => void;
};

export function Antechamber({ apiKey, session, loadedPerson, onComplete }: Props) {
  const { state, currentQuestion, submitAnswer, submitIntention, canUndo, undo, seer, engine } = useAntechamberEngine({
    apiKey,
    sessionId: session.id,
  });

  // Loaded-from-save bootstrap: on mount, if we have a saved Person,
  // hydrate the engine directly and skip the antechamber.
  ...
  const [guessStall, setGuessStall] = useState<{ text: string; ts: number } | null>(null);
  useEffect(() => {
    if (!guessStall) return;
  ...
  function handleResume(match: ReturningMatch) {
    ...
    // saved snapshot (skip the antechamber, jump to intention prompt).
  ...
  // and gets a fresh reading from the same antechamber state.
  //
  ...
  useEffect(() => {
    ...
    publishAntechamberState(state);
  }, [state, state.thinking, state.picks_log.length, state.queue, state.asked_node_ids]);

  // Clear the bus snapshot when Antechamber unmounts.
  useEffect(() => () => publishAntechamberState(null), []);
  useEffect(() => () => {
  ...
  if (farewell !== 'idle') {
    ...
    // v3: dead-end signals fired during the antechamber. The reading would
    ...
    dialogText = guessStall.text.toLowerCase();
    dialogKey = `stall-${guessStall.ts}`;
  } else if (centenarian) {
  ...
  return (
    <div className="screen screen--antechamber">
      ...
      <div className={
        showGag
          ? 'antechamber__dialogue-host antechamber__dialogue-host--gag'
          : isIntentMoment
            ? 'antechamber__dialogue-host antechamber__dialogue-host--intent'
            : 'antechamber__dialogue-host'
      }>
        {sensing ? (
      ...
      <div className="ui-frame ui-frame--antechamber">
        <div className="ui-frame__choices">
            ...
            <WarmColdChoice
              ...
              onPick={(v) => {
                ...
                if (inst?.kind === 'guess') {
                  ...
                  if (stall) setGuessStall({ text: stall, ts: Date.now() });
                }
                void submitAnswer(v);
              }}
            />
          ...
          {isAwaitingIntention && confirm === 'done' && (
            <div className="antechamber__intentions">
              {lastIntention && (
                <p className="antechamber__last-intention">
      ...
      <div className="antechamber__footer">
        ...
        {state.stage === 'reading_ready' && seer && farewell === 'idle' && (
          <button
            className="btn btn--big antechamber__enter"
            onClick={handleEnter}
...
function cloneProfile(p: AntechamberProfile): AntechamberProfile {
  return JSON.parse(JSON.stringify(p)) as AntechamberProfile;
}
