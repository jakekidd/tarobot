...
import { parsePillarsMd, slugify } from '../src/pipeline/antechamber/parsePillarsMd';

describe('parsePillarsMd', () => {
  it('parses a minimal Pillar choice question', () => {
    const md = `# Antechamber
...
Options:
  - a
`;
    expect(() => parsePillarsMd(md)).toThrow(/unknown format/);
  });
...
Options:
  - a
`;
    expect(() => parsePillarsMd(md)).toThrow(/must be 'left \| right'/);
  });
