// Headless survey driver — a CLI rig for walking through the survey
...
// detective's next_move per turn. Useful for:
...
import type { EngineState, RenderedQuestion } from '../src/pipeline/survey/types';
import type { LivingDoc, CoverageMap } from '../src/pipeline/survey/living-doc';
...
async function runInit(sidArg?: string): Promise<void> {
  ...
  const engine = new SurveyEngine({ adapter, session_id: sid });
...
async function runStep(sid: string, answer: string): Promise<void> {
  ...
  const peekEngine = new SurveyEngine({ adapter: tmpAdapter, session_id: sid, initialState: state });
  ...
  const engine = new SurveyEngine({ adapter, session_id: sid, initialState: state });
  ...
  printDetectiveMoveFromEvents(events);
  console.log('');
...
function printUsage(): void {
  console.log(`survey-driver — headless survey rig with x-ray vision
...
function printDetectiveMoveFromEvents(events: readonly AgentEvent[]): void {
  // Find the most recent detective_step event and try to parse the
  // response_preview for next_move + leading_hypothesis.
  const det = [...events].reverse().find((e) => e.label === 'detective_step' && e.response_preview);
